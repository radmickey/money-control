...
import { login, telegramLogin, clearError } from '../store/slices/authSlice';
...
const Login: React.FC = () => {
  ...
  const { isTelegram, initData, user: tgUser } = useTelegramContext();
  ...
  useEffect(() => {
    if (isTelegram && initData && !isAuthenticated && !loading) {
      dispatch(telegramLogin(initData));
    }
  }, [isTelegram, initData, isAuthenticated, loading, dispatch]);
  ...
  const handleGoogleLogin = () => {
    window.location.href = `${import.meta.env.VITE_API_URL || 'http://localhost:9080/api/v1'}/auth/google`;
  };
  ...
  return (
    <div className="min-h-screen flex bg-[#0a0b14]">
        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex items-center justify-between">
              <label className="flex items-center gap-2 cursor-pointer group">
                <input
                  type="checkbox"
                  className="w-4 h-4 rounded border-zinc-600 bg-zinc-900 text-violet-500 focus:ring-violet-500/20 focus:ring-offset-0"
                />
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-[#0a0b14] text-zinc-500">or continue with</span>
            </div>
          </div>

          <button
            onClick={handleGoogleLogin}
            className="w-full flex items-center justify-center gap-3 px-4 py-3.5 rounded-xl border border-zinc-700/50 bg-zinc-900/50 text-white hover:bg-zinc-800/50 hover:border-zinc-600/50 transition-all"
          >
      ...
      <div className="hidden lg:flex flex-1 items-center justify-center relative overflow-hidden">
        {/* Gradient background */}
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-600/30 via-violet-600/20 to-fuchsia-600/10" />

        {/* Animated orbs */}
        <div className="absolute inset-0">
          <div className="absolute top-1/4 left-1/4 w-[400px] h-[400px] bg-indigo-500/30 rounded-full blur-[100px] animate-pulse" />
          <div className="absolute bottom-1/4 right-1/4 w-[350px] h-[350px] bg-violet-500/20 rounded-full blur-[100px] animate-pulse" style={{ animationDelay: '1s' }} />
        </div>
        ...
        <div className="relative text-center px-12">
          ...
          <p className="text-xl text-zinc-300 max-w-md mx-auto">
            Track all your assets in one place. Stocks, crypto, real estate, and more.
          </p>
        </div>
      </div>
    </div>
